...
- **PDF 파일 연결(Finder 더블클릭/"다음으로 열기") 완료 — 사용자 실기 확인(2026-07-17)**. 두 층의 수정이 필요했음:
...
- **"연결 프로그램" 지정 시 앱 이름이 "ui"로 표시되던 버그 수정(2026-07-17, 실물 Windows 실기 확인은 대기)**: winres가 `ProductName`/`FileDescription`을 명시하지 않으면 `CARGO_PKG_NAME`(크레이트 이름 "ui")으로 기본 채움(winres 0.1.12 lib.rs로 확인) — `crates/ui/build.rs`에서 `res.set("ProductName", "PDF Outliner")` 등 4개 필드를 명시해 해결. 파일 경로 전달은 macOS와 달리 argv 방식이라 기존 코드로 이미 동작(사용자 확인).
- **VM(UTM 등) 미지원 — 사용자 확정(2026-07-17)**: UTM(맥 위 Windows 게스트)에서 실행하면 시작 즉시 `Error: Wgpu(NoSuitableAdapterFound)`로 종료(2026-07-13 실측; `WGPU_BACKEND=gl` 강제도 동일 실패 — VM에 3D 가속 자체가 없음). **PDF 조작(pdfium/lopdf)은 전부 CPU 작업이라 wgpu와 무관** — wgpu는 GUI를 화면에 그리는 백엔드일 뿐이고, 창을 만드는 단계에서 그래픽 어댑터를 못 찾아 PDF 코드가 실행되기도 전에 죽는 것. 실물 Windows에선 v0.1.2부터 정상 확인됐으므로 VM 지원은 하지 않기로 확정. 나중에 재검토한다면 아래 두 방향이 후보 — **어느 쪽이든 대상 VM에서 실측이 선행돼야 하며, 검증 수단 없이 폴백 코드를 미리 넣지 않는다는 원칙(2026-07-13 결정) 유지**:
  ...
  - 결론(기록용): 이 프로젝트 사정(실기 검증 의존, wgpu 기준으로 튜닝된 텍스처 한도 로직, 렌더링 회귀 경험)에는 **방법 2가 더 적합** — 코드 경로가 하나로 유지되는 가치가 큼. 방법 1은 "아주 낡은 실물 하드웨어까지 지원"이 목표일 때 의미가 있고, 그 경우에도 llvmpipe 동봉까지 가야 완전해짐.
...
- [ ] **Sumatra식 탭 구조(여러 문서 동시 오픈/조작/저장) — 미착수, 사전 분석만 완료(2026-07-17, 사용자 질문에 답한 내용 기록)**. 근본적 기술 장벽은 없음 — 난이도의 본질은 "어려운 문제 하나"가 아니라 **넓은 파급 범위 × 실기 검증 부담**:
  ...
  - **착수한다면 3단계 권장**: (1) 탭 없이 `DocumentTab` 구조체로 상태만 분리, 동작 불변 확인 → (2) 탭바+다중 오픈 → (3) 생명주기/복구/세션. 여러 세션짜리 규모, 단계마다 실기 검증 필요.
...
- [x] **한글 파일명 자소 분리(최근 파일 목록 등) — 완료, 사용자 실기 확인**. 원인: macOS 파일시스템(구형 HFS+는 강제, APFS는 강제는 안 하지만 외부 유입 파일엔 여전히 남아있을 수 있음)이 한글처럼 분해 가능한 유니코드를 파일명에 **NFD(분해형)** 로 담을 수 있는데, egui 텍스트 셰이핑은 NFC(조합형)를 전제로 해서 그대로 그리면 자소가 낱개로 갈라져 보임. `app.rs`에 `pub(crate) fn display_filename(path)` 신설(`unicode-normalization` 크레이트의 `.nfc()`로 정규화) — 기존에 6곳(`open_recent_file`, `poll_file_rename`, `current_filename_for_export`, `update_window_title`, 크래시 복구 대화상자, `toolbar.rs` 최근 파일 목록)에 중복돼 있던 "파일명 추출" 보일러플레이트를 이 함수 하나로 통합하면서 정규화도 같이 적용. 실제 경로(`PathBuf`)는 OS가 준 그대로 유지(파일 I/O·비교엔 영향 없음) — 화면 표시용 `String`을 뽑을 때만 정규화. Windows(NTFS)는 이미 대개 NFC라 사실상 no-op이라 플랫폼 분기 없이 항상 적용. NFD 바이트를 명시적으로 만들어 재현 후 창 제목이 정상 조합형으로 뜨는 것으로 검증.

### 뷰어 개선 라운드 (2026-07-18 접수 — 구현 완료분 + 대기 중 3건, ⚠️ 미커밋 작업 있음)
**구현 완료(사용자 실기 확인)**: (1) 검색 순회 시 사이드바가 해당 북마크로 스크롤(`jump_to_search_match`가 `scroll_sidebar_to_active_once` 재사용), (2) 검색 하이라이트 `expand(2.0)`(pdfium rect가 advance 기준이라 좁음 — 사용자가 "이 정도로 종결" 확정), (3) 폭 맞춤/쪽 맞춤 아이콘 버튼(`toolbar.rs`의 `icon_button`+수제 벡터 아이콘, `request_fit_page`는 viewer_panel에서 소비), (4) **연속 스크롤 모드**('C' 전환, `show_continuous`): ScrollArea `show_viewport` 가상화(보이는 페이지 ±1만 렌더·캐싱, `continuous_textures`), 텍스트 선택/복사(`selection_page`로 페이지 귀속, 한 페이지 내만)/링크 클릭 지원, 검색 하이라이트는 범위 밖. 여기서 잡은 함정들: **egui `scroll_to_rect`의 target은 화면 좌표**(content 좌표를 넘기면 오프셋 중복 가산으로 문서 끝으로 clamp — "이동 무반응"처럼 보였음); 이후 아예 `vertical_scroll_offset`(그리기 전 적용, 즉시 점프)으로 교체해 애니메이션 경유도 제거; **줌 변경 시 레이아웃 높이가 비례 변화하므로 스크롤 오프셋도 같은 비율로 재조정**해야 페이지 점프 안 함(`continuous_last_page_width`); 같은 감지로 줌 중엔 재렌더링 스킵(기존 텍스처 스케일 표시, 줌 멎으면 1회 재렌더) — 핀치 중 매 프레임 pdfium 렌더링이 심한 버벅임의 주범이었음. 사이드바 `scroll_to_me` 부드러움은 애니메이션 시간(400ms) 동안 강제 repaint로 해결(`scroll_animation_until` — egui는 입력 없으면 리페인트 안 함).
**대기 중(2026-07-18 사용자 리포트, 다음 작업)**:
- [ ] 연속 모드 진입 시 페이지가 뷰어 영역 안에서 **오른쪽으로 살짝 이동**해 보임 — 가로 중앙 정렬 오프셋(`x_center_offset`, clip_rect 기준) 계산이 쪽 단위 모드의 중앙 위치와 미묘하게 다른 것으로 추정(스크롤바 폭 등). 쪽 단위와 같은 x 위치가 되도록 맞출 것.
- [ ] **연속 모드 이동 속도**: debug 빌드에서 북마크 클릭 → 도착까지 ~3초(캐시가 없는 지점 점프 시 2~4페이지 동기 렌더링 때문, 몇 번 누르면 캐시로 1초 미만). release 배포판으로 재판단 예정 — 여전히 느리면 "프레임당 1페이지씩 나눠 렌더링"(도착 페이지 먼저, 이웃은 다음 프레임) 방식으로 개선. PDFium 메인 스레드 제약(§7) 때문에 백그라운드 렌더링은 불가.
- [ ] **페이지 이동 시작 시 사이드바 자동 재중앙**: 사용자가 사이드바를 스크롤해 선택 북마크가 안 보이는 상태에서 페이지 이동을 시작하면, 현재 페이지의 북마크가 사이드바 중앙으로 부드럽게 스크롤돼 보이게. 기존 `scroll_selected_into_view`(사이드바 키보드 순회용) 메커니즘을 "페이지 변경으로 selected_bookmark가 자동 동기화될 때"도 세우면 됨 — 단 사용자가 사이드바를 직접 스크롤하는 중에 싸우지 않게 "페이지가 실제로 바뀐 프레임"에만.

### 포커스 모델/사이드바 UX 재설계 (2026-07-17 접수, 전부 완료 — 사용자 실기 확인)
