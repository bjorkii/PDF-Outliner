...
**아직 실기 미확인 (다음 세션 최우선)**: 가장 최근에 고친 wgpu 크래시 수정("저장"/"저장하지 않음" 클릭 시 앱이 꺼지던 문제) — 원인은 확정됐고 수정도 했지만, 재실행해서 진짜 안 꺼지는지 확인 안 됨. §6 참고.
...
- **PDF 자체 저장**("저장" 버튼, 미저장 변경 시 저장/저장하지않음/취소 확인창): 저장 로직 자체는 headless 테스트로 강하게 검증됨(§4). **UI에서 앱이 꺼지던 크래시의 원인을 확정하고 수정함**(wgpu 텍스처 파괴 타이밍 문제, 자세한 원인은 §7) — **재실행해서 진짜 해결됐는지 확인이 다음 세션 최우선 작업.**
