...
- **한글 IME 조합 끊김 — 확정된 업스트림 winit 버그로 결론, 더 이상 자체 수정 시도 안 함(2026-07-13)**: 정확한 재현 조건(사용자가 정밀하게 좁혀줌, 0.29.1 기준): 앱을 새로 띄운 뒤 **이번 세션에서 처음으로** 아무 텍스트 필드(북마크든 검색창이든)에 포커스를 주고 입력하면 그 첫 글자가 자모로 완전히 분리됨(예: 검색창에 "송" → "ㅅㅗㅇ"). 그 상태에서 **두 번째로** 다른 필드로 옮겨가 입력하면, 첫 번째 필드에서 입력했던 마지막 자소가 새 필드에 먼저 "flush"되어 나타난 뒤 새로 입력한 글자가 정상 조합됨. 즉 IME composition/commit 이벤트가 지금 포커스를 가진 위젯이 아니라 **이전에 포커스를 가졌던 위젯에 속한 채로 지연 전달**되는 것으로 보이는 패턴. 그 이후의 입력은 정상 — 실사용에 거슬리지만 좁은 범위의 문제.
