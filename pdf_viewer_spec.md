...
- **한글 IME 조합 끊김 — egui 0.35 업그레이드로 고쳐보려 했으나 오히려 악화, 0.29.1로 롤백함(미해결 상태 유지, 2026-07-13)**: 정확한 재현 조건(사용자가 정밀하게 좁혀줌, 0.29.1 기준): 앱을 새로 띄운 뒤 **이번 세션에서 처음으로** 아무 텍스트 필드(북마크든 검색창이든)에 포커스를 주고 입력하면 그 첫 글자가 자모로 완전히 분리됨(예: 검색창에 "송" → "ㅅㅗㅇ"). 그 상태에서 **두 번째로** 다른 필드로 옮겨가 입력하면, 첫 번째 필드에서 입력했던 마지막 자소가 새 필드에 먼저 "flush"되어 나타난 뒤 새로 입력한 글자가 정상 조합됨. 즉 IME composition/commit 이벤트가 지금 포커스를 가진 위젯이 아니라 **이전에 포커스를 가졌던 위젯에 속한 채로 지연 전달**되는 것으로 보이는 패턴. 그 이후의 입력은 정상 — 실사용에 거슬리지만 좁은 범위의 문제.
  ...
  - **여전히 미해결**: 근본 원인은 egui-winit의 IME 상태 관리(포커스 전환 시 이전 위젯의 커밋 이벤트가 새 위젯으로 새는 것으로 추정)로 보이나, 프레임워크 전체를 업그레이드하는 건 이미 실패가 증명된 경로 — 재시도하려면 프레임워크를 통째로 올리지 말고 egui-winit의 IME 관련 소스만 좁게 파서 우리 코드 레벨의 대응이 가능한지부터 볼 것(§7 교훈 참고).
