...
- **폴더 일괄 북마크 import & 저장 (2026-07-19 구현 완료, `crates/ui/src/batch_import.rs`)** — 툴바 "북마크 가져오기" → "폴더 일괄 적용…"으로 폴더 + 북마크 csv/xlsx를 고르면 폴더 안 모든 PDF를 recursive하게 순회하며(심볼릭 링크 디렉토리는 순환 방지로 제외) 파일명이 매칭되는 북마크를 각 PDF에 저장. 구현 결정 사항:
  ...
  - **UI**: 뷰어 영역을 통째로 전환(스펙 1순위안) — 확인 화면(발견 PDF 수 + 매칭 파일 수 + 시작/취소) → 진행 화면(ProgressBar + 실시간 로그, `stick_to_bottom`) → 완료 화면(통계 + 닫기). `batch_import::poll`이 프레임당 1파일 처리 + `request_repaint`(§7 poll_search_job과 같은 프레임 분할 패턴, `app.rs`의 `poll_batch_import`가 매 프레임 호출). 진행 중 메뉴 재진입은 비활성화.
  ...
  - **검증**: 단위 + e2e 테스트 4개(`batch_import.rs` tests — 실제 샘플 PDF로 성공/손상 파일 복원/skip/로그 파일 생성, NFD↔NFC 매칭, 열린 파일 skip). GUI 흐름(메뉴→다이얼로그→로그 뷰) 실 점검은 미확인.
