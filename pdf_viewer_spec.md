...
- **폴더 일괄 북마크 import & 저장 (2026-07-19 구현 완료, `crates/ui/src/batch_import.rs`)** — 툴바 "북마크 가져오기" → "폴더 일괄 적용…"으로 **폴더 하나만** 고르면, 그 안의 모든 PDF를 recursive하게 순회하며(심볼릭 링크 디렉토리는 순환 방지로 제외) 파일명이 매칭되는 북마크를 각 PDF에 저장. 구현 결정 사항:
  - **북마크 파일 자동 인식(2026-07-19 사용자 지정으로 변경)**: 북마크 csv/xlsx를 다이얼로그로 따로 묻지 않는다 — 폴더 안(하위 폴더 포함)의 모든 `.xlsx`/`.csv`를 자동 인식해 **xlsx 전부 → csv 전부** 순서(각각 경로 사전순)로 읽는다. 같은 PDF('파일명')의 행이 여러 북마크 파일에 있으면 **먼저 읽은 파일이 이기고 뒤의 중복은 건너뛰며** `setup_notes`로 확인 화면과 .log에 기록. 스키마가 안 맞아 파싱 실패하는 엑셀/CSV는 그 파일만 무시하고 경고 기록. 숨김 파일(`.` 시작)과 Excel 잠금 파일(`~$` 시작)은 수집에서 제외.
  - **파일별 처리**: (1) `x.pdf` → `x.pdf.backup` rename, (2) 그 백업을 소스로 `write_bookmarks_incremental(&backup, &x.pdf, tree)` — lopdf `IncrementalDocument`는 원본 바이트 전체 + 새 리비전을 기록하므로 결과 파일이 그 자체로 완전해서 설계 메모의 "복사" 단계가 필요 없었음. (3) pdfium 엔진이 살아있으면 결과를 재오픈해 파싱 검증(실패 시 결과 삭제 + 백업을 원래 이름으로 복원 — 단일 파일 `save_bookmarks_to_pdf`와 같은 안전 관례). 쓰기 실패 시에도 백업 복원. pdfium 없이도 파이프라인 전체가 동작(검증만 생략).
  - **skip 정책(전부 파일을 안 건드림)**: `.backup`이 이미 있으면 이전 실행 흔적으로 보고 skip / '파일명' 매칭 행 없으면 skip / 현재 앱에 열려 있는 파일은 rename이 문서 핸들·macOS inode 감시와 충돌하므로 skip(canonicalize 비교). 매칭은 NFC 정규화(`display_filename`과 동일 규칙, 행 쪽도 `.nfc()`)로 macOS NFD 파일명과 Excel 입력이 어긋나지 않게.
  - **UI**: 뷰어 영역을 통째로 전환(스펙 1순위안) — 다이얼로그는 폴더 선택 하나뿐이고 이후는 전부 뷰어 화면: 확인 화면(발견 PDF 수 + 자동 인식된 북마크 파일 목록·적용 순서 + 매칭 파일 수 + 시작/취소) → 진행 화면(ProgressBar + 실시간 로그, `stick_to_bottom`) → 완료 화면(통계 + "되돌아가기" — 배치 시작 시점에 열려 있던 문서/페이지(`BatchImportJob::return_to`)로 복귀, 열린 문서가 없었으면 "닫기"로 표시. 2026-07-19 요청). **주의(2026-07-19 실 점검 리포트로 수정)**: 처음엔 폴더 다이얼로그 직후 북마크 파일 다이얼로그를 연달아 띄웠는데, macOS 네이티브 패널엔 `set_title`이 표시되지 않아 두 번째 다이얼로그가 "잘못 뜬 개별 파일 선택 모드"로 오인돼 진행 불가 판정 — 이 리포트를 계기로 사용자가 북마크 파일 자동 인식 방식을 지정(위 항목). 네이티브 다이얼로그를 연속으로 띄우는 UX는 재도입하지 말 것. `batch_import::poll`이 프레임당 1파일 처리 + `request_repaint`(§7 poll_search_job과 같은 프레임 분할 패턴, `app.rs`의 `poll_batch_import`가 매 프레임 호출). 진행 중 메뉴 재진입은 비활성화.
  - **완료 통계 + .log**: "전체 N개 중 성공/실패/건너뜀" 통계 표시, 대상 폴더 루트에 `bookmark-import-YYYYMMDD-HHMMSS.log`(UTF-8, 파일별 결과·사유 + 통계) 생성 — 타임스탬프 덕에 재실행이 이전 로그를 덮어쓰지 않음.
  - **검증**: 단위 + e2e 테스트 5개(`batch_import.rs` tests — 실제 샘플 PDF로 성공/손상 파일 복원/skip/로그 파일 생성, NFD↔NFC 매칭, 열린 파일 skip, 북마크 파일 자동 인식·xlsx 우선·중복 건너뜀). **GUI 흐름 사용자 실 점검 확인 완료(2026-07-19)** — 첫 실 점검은 다이얼로그 연속 팝업 문제로 진입 단계에서 중단됐으나, 북마크 파일 자동 인식 방식으로 바꾼 뒤 "이상 없음" 확인받음. 그 직후 추가한 완료 화면 "되돌아가기" 버튼만 실 점검 미확인.
