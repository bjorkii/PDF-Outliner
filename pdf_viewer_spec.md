...
- [ ] **연속 모드 스크롤 중 페이지 경계에서 한 번 "덜컹"(속도가 줄었다 회복) — 완화 구현(2026-07-18, 실 점검 대기)**: 원인은 경계에서 새 페이지가 가시 범위에 들어오는 프레임에 pdfium 렌더링+비트맵 복사+텍스처 업로드가 동기로 일어나 그 프레임만 길어지는 것(PDFium 메인 스레드 제약 §7 때문에 백그라운드 불가). 완화: 스크롤 진행 중(ScrollArea state의 `velocity()` 또는 `smooth_scroll_delta` 감지)엔 **반해상도**(`target_width/2`, 픽셀 1/4)로 렌더링해 프레임 시간을 줄이고, 줌·스크롤이 다 멎으면 해상도 안 맞는 페이지를 **프레임당 1장씩** 원해상도로 업그레이드(+`request_repaint`로 업그레이드 체인 유지 — 즉시모드 함정). 그래도 남는 미세한 덜컹은 한계로 수용할지 실 점검 후 판단.
