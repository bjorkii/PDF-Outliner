...
- [x] **페이지 이동 시작 시 사이드바 자동 재중앙 — 구현 완료(2026-07-18, 실 점검 대기)**: `app.rs`에 `sidebar_reveal_selected_once` 1회성 플래그 신설 — `set_current_page`(페이지가 실제로 바뀔 때만, clamp로 제자리인 경우 제외)와 `note_visible_page_during_scroll`(연속 스크롤 페이지 추적)이 세우고, `sidebar.rs`가 소비하면서 접힌 조상 펼침 + `DragState::scroll_selected_into_view`(화면 밖일 때만 부드럽게 중앙 스크롤 — 이미 검증된 키보드 순회용 메커니즘 재사용)로 이어줌. 이미 보이는 항목은 안 건드려서 화살표 페이지 연타 중 사이드바가 들썩이지 않음.
